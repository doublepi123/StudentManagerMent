...
public class LoginWindowControl {
    ...
    private boolean checkUsernameAndPassword(){
        ...
        ResultSet rs = sqlHelper.executeQuery(
                "SELECT PASSWORD FROM USER WHERE USERNAME = '"+tUsername.getText()+ "';"
        );
        try {
    ...
    public void Login(ActionEvent actionEvent) throws IOException, SQLException {
        ...
        mainWindowControl.welcome.setText(mainWindowControl.welcome.getText()+tUsername.getText());
        stage.show();
        sqlHelper.close();
        primaryStage.close();
    }
    ...
    public void submission(ActionEvent actionEvent) throws IOException {
        ...
        if(sqlHelper.executeUpdate(
                "INSERT INTO USER VALUES('"+tUsername.getText()+"','"+tPassword.getText()+"');"
        ) == 1){messageBox.setMessage("您输入的信息有误！");
            messageBox.setMessage("注册成功！");
            messageBox.showMessage();
            sqlHelper.executeUpdate("CREATE TABLE "+tUsername.getText()+
                    "_STUDENT (SNO CHAR(12) PRIMARY KEY,SNAME VARCHAR(20),SEX CHAR(4), SDEPT VARCHAR(30));");
            sqlHelper.executeUpdate("CREATE TABLE "+tUsername.getText()+
                    "_COURSE (CNO CHAR(12) PRIMARY KEY,CNAME VARCHAR(20),CREDIT INT);");
            sqlHelper.executeUpdate("CREATE TABLE "+tUsername.getText()+
                    "_SC (SNO CHAR(12),CNO CHAR(12));");
    ...
    public void help(ActionEvent actionEvent) throws IOException {
        messageBox = new MessageBox();
        messageBox.setMessageAndShow("本程序数据库服务器架设在上海阿里云，使用前请保证网络畅通");
    }
